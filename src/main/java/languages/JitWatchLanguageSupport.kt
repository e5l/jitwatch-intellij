...
fun <CT : PsiElement, MT : PsiElement> JitWatchLanguageSupport<CT, MT>.matchesSignature(
    method: MT,
    metaMember: IMetaMember
): Boolean {
    return matchesSignature(
        method,
        metaMember.memberName,
        metaMember.paramTypeNames.toList(),
        metaMember.returnTypeName
    )
}

fun <CT : PsiElement, MT : PsiElement> JitWatchLanguageSupport<CT, MT>.matchesSignature(
    method: MT,
    signature: MemberSignatureParts
): Boolean {
    return matchesSignature(method, signature.memberName, signature.paramTypes.toList(), signature.returnType)
}

fun PsiElement.matchesSignature(metaMember: IMetaMember) =
    LanguageSupport.forElement(this).matchesSignature(this, metaMember)
